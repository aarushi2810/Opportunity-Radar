...
def main():
  ...
  +-----------------------------------------------------------+
    ...
    print(f"  Pipeline:       http://localhost:{config.PORT}/api/pipeline/status")
    print(f"  Dashboard Stats:http://localhost:{config.PORT}/api/dashboard/stats")
    ...
    print(f"  Database:       {'PostgreSQL' if config.DATABASE_URL else 'SQLite (local dev)'}")
    print(f"  Redis:          {'Connected' if config.REDIS_URL else 'fakeredis (local dev)'}")
    print()
